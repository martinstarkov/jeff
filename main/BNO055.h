...
class BNO055 {
    ...
    public:
      BNO055(TwoWire* wire, uint8_t address) {
        ...
        if (!instance->begin()) {
          Bluetooth::print(FAILURE + "BNO sensor with address=" + String(address, HEX) + ", wire=" + String(int(wire)) + " not detected. Check wiring / I2C address");
        } else {
          name = "BNO055-" + String(address, HEX) + "-" + String(int(wire));
          status = true;
          Bluetooth::print(SUCCESS + "BNO sensor with address=" + String(address, HEX) + ", wire=" + String(int(wire)) + " initialized. Example data (temperature): " + String(instance->getTemp()));
        }
      }
